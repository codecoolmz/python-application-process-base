class Applicant():
    ...
    def _4_specific_applicant_by_first_name(cls):
        return[{'full_name': i.first_name + ' ' + i.last_name} for i in cls.get_all() if i.first_name == 'Carol']
    ...
    def _5_specific_applicant_by_email_domain(cls):
        return[{'full_name': i.first_name + ' ' + i.last_name} for i in cls.get_all() if '@adipiscingenimmi.edu' in
               i.email]
    ...
    def _6_inserting_a_new_applicant(cls):
        from db import Database
        Database.applicants_data.append([11, 'Markus', 'Schaffarzyk', '003620/725-2666', 'djnovus@groovecoverage.com',
                                        54823])
        return [i.__dict__ for i in cls.get_all() if i.id == 11]
    ...
    def _7_updating_data(cls):
        ...
        for i in Database.applicants_data:
            if i[1] == 'Jemima' and i[2] == 'Foreman':
                i[3] = '003670/223-7459'
        return[{'phone_number': i.phone_number} for i in cls.get_all() if i.first_name == 'Jemima' and i.last_name ==
               'Foreman']
    ...
    def _8_deleting_applicants(cls):
        from db import Database
        Database.applicants_data = [i for i in Database.applicants_data if 'mauriseu.net' not in i[4]]
        return len([i for i in cls.get_all() if 'mauriseu.net' in i.email])
